...
def main(args):
    ...
    for i, img_path in enumerate(img_list_gt):
        ...
        # img_gt = cv2.imread(img_path).astype(np.float32) / 255.
        if args.suffix == '':
        ...
        # img_restored = cv2.imread(img_path_restored).astype(np.float32) / 255.
        # img_gt = cv2.resize(img_gt,img_restored.shape[:2][::-1])
        print(img_path,img_path_restored)
