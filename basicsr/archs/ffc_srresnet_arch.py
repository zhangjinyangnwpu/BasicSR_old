...
class FFC_MSRResNet(nn.Module):
    def __init__(self, num_in_ch=3, num_out_ch=3, num_feat=64, num_block=5, upscale=4):
        super(FFC_MSRResNet, self).__init__()
        self.upscale = upscale

        self.conv_first = nn.Conv2d(num_in_ch, num_feat, 3, 1, 1)
        self.body = make_layer(Bottleneck, num_block,inplanes=num_feat, planes=num_feat)
    ...
    def forward(self, x):
        ...
        out = torch.cat((feat_l,feat_g),dim=1)
