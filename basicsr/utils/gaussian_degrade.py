...
class SRMDPreprocessing(object):
    ...
    def __call1__(self, hr_tensor, random=True):
        with torch.no_grad():
            # only downsampling
            if self.gen_kernel.blur_type == 'iso_gaussian' and self.gen_kernel.sig == 0:
...
if __name__ == '__main__':
    # anisotropic iso_gaussian
    degrade = SRMDPreprocessing(scale=4,kernel_size=21,blur_type='iso_gaussian',sig=4,noise=1)
    from PIL import Image
    img = Image.open('../../results/test_data/baby.png')
    img = torchvision.transforms.ToTensor()(img)
    img = img.unsqueeze(0)
    # img = img.unsqueeze(0)
    hr = img.cuda()
    lr, kernel = degrade(hr,random=False)
    # print(lr.shape,torch.max(kernel),torch.min(kernel))
    # lr = lr.squeeze(0)
    lr = lr.squeeze(0)
    res = torchvision.transforms.ToPILImage()(lr)
    res.save('../../results/test_data/lr.png')